...
public class ConsultarFilmesUseCase {
    ...
    public List<Filme> findAll() {
        List<Filme> filmesInativosList = filmeDAO.findAll().stream()
                .filter(filme -> filme.getStatus() != FilmeStatus.ATIVO)
                .collect(Collectors.toList());

        if(filmesInativosList.isEmpty()) {
            throw new EntityNotFoundException("Não há filmes cadastrados ativos");
        }

        return filmesInativosList;
    }
}
